...
import { Target, Loader2, AlertTriangle, Activity, CheckCircle } from 'lucide-react';
const apiKey = "";
...
export default function App() {
  ...
  const scanMarket = async () => {
    ...
    const prompt = `You are an expert e-commerce analyst for Arab dropshipping markets. Today is ${today}. Find ${productCount} REAL winning products in "${market}" for "${niche}". Return ONLY valid JSON: {"products":[...]} Each product: product_name (Arabic), product_name_en (English), category (Arabic), why_winning (Arabic 2 sentences), cost_price (USD number), selling_price (USD number), profit_margin (USD number), saturation (Arabic), fb_search_query, aliexpress_query (English), data_source, verification_status (Arabic), active_ad_examples (string[]), competitor_count, ad_spend_estimate (string), target_audience (Arabic). No text outside JSON.`;
    setScanStep(2);
    try {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="text-center mb-8"><h1 className="text-4xl font-black text-slate-800">ALI Spy Pro <span className="text-lg text-indigo-500">V6.0</span></h1><p className="text-slate-500 mt-1">صور حقيقية للمنتجات عبر Bing Images</p></div>
